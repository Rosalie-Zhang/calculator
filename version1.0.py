...
#设置窗口不同部分的颜色
button_bg = 'white'
math_sign_bg = 'DarkGray'
cal_output_bg = 'Gainsboro'
...
entry = tkinter.Entry(root, justify="right", font=1,width=65)
...
def place_button(text, func, func_params, bg=button_bg, **place_params):
    ...
    my_button = partial(tkinter.Button, root, bd=0,bg=button_bg,font=('Helvetica','10'),padx=10,pady=3,width=15,height=2,activebackground=button_active_bg)
...
# 数值输入类按钮
place_button('1', get_input, (entry, '1'), row=3, column=0, padx=5,pady=5)
place_button('2', get_input, (entry, '2'), row=3, column=1, padx=5, pady=5)
place_button('3', get_input, (entry, '3'), row=3, column=2, padx=5, pady=5)
place_button('4', get_input, (entry, '4'), row=3,column=3, padx=5, pady=5)
place_button('5', get_input, (entry, '5'), row=4, column=0, padx=5, pady=5)
place_button('6', get_input, (entry, '6'), row=4, column=1, padx=5, pady=5)
place_button('7', get_input, (entry, '7'), row=4, column=2, padx=5, pady=5)
place_button('8', get_input, (entry, '8'), row=4, column=3, padx=5, pady=5)
place_button('9', get_input, (entry, '9'), row=5, column=0, padx=5, pady=5)
place_button('0', get_input, (entry, '0'), row=5, column=1, padx=5, pady=5)
place_button('.', get_input, (entry, '.'), row=5, column=2, padx=5, pady=5)
...
#功能输入类按钮(背景色、触发功能不同)
place_button('<-', backspace, (entry,), row=3, column=4,padx=5, pady=5)
place_button('C', clear, (entry,), row=4, column=4, pady=5,padx=5)
place_button('=', calc, (entry,), bg=cal_output_bg, row=5, column=3,padx=5, pady=5,
                 columnspan=2, sticky=tkinter.E + tkinter.W + tkinter.N + tkinter.S)
place_button('﹢', get_input, (entry, '+'), bg=cal_output_bg, row=6, column=0, padx=5, pady=5)
place_button('﹣', get_input, (entry, '-'), bg=cal_output_bg, row=6, column=1, padx=5, pady=5)
place_button('×', get_input, (entry, '*'), bg=cal_output_bg, row=6, column=2, padx=5, pady=5)
...
#运算类型按钮
place_button('生命的剩余时间', get_input, (entry, '输入你的生日（XXXX.XX.XX）:'), bg=math_sign_bg, row=2, column=0, padx=5, pady=5)
place_button('距离DDL的剩余时间', get_input, (entry, '输入DDL的具体日期（XXXX.XX.XX）:'), bg=math_sign_bg, row=2, column=1, padx=5, pady=5)
place_button('节日', get_input, (entry,'输入节日日期（XX.XX）:'), bg=math_sign_bg, row=2, column=2, padx=5, pady=5)
place_button('n天后的日期', get_input, (entry, '输入天数:'), bg=math_sign_bg, row=2, column=3, padx=5, pady=5)
place_button('事件发生距今时间', get_input, (entry, '输入事件发生日期（XXXX.XX.XX）:'), bg=math_sign_bg, row=2, column=4, padx=5, pady=5)
place_button('二进制', turnbin, (entry,), bg=math_sign_bg,row=7, column=0, padx=5, pady=5)
place_button('取整', turnint, (entry,), bg=math_sign_bg,row=6, column=4, padx=5, pady=5)
place_button('实时美元汇率', exchange, (entry,), bg=math_sign_bg,row=7, column=1,padx=5, pady=5,
            columnspan=2, sticky=tkinter.E + tkinter.W + tkinter.N + tkinter.S)
place_button('平均值和方差', statistics, (entry,), bg=math_sign_bg,row=7, column=3,padx=5, pady=5,
            columnspan=2, sticky=tkinter.E + tkinter.W + tkinter.N + tkinter.S)
